...
const AdminPage = () => {
    ...
    return 'Search available on Sessions or Members';
    ...
    const filteredSessionMembers = useMemo(() => {
        ...
        const handleGenerateReport = async () => {
            try {
                ...
                for (const session of sessionsList) {
                    const attRes = await api.get(`/api/attendance/session/${session._id}`);
                    reportData.push({
                        ...
                        present: attRes.data.count || 0,
                        attendance: attRes.data.attendance || []
                    });
                }

                let csv = 'Session,Date,Venue,Type,Status,Present Count\n';
                reportData.forEach(record => {
                    csv += `"${record.session}","${record.date}","${record.venue}",`;
                    csv += `"${record.type}","${record.status}","${record.present}"\n`;
                }

            csv += '\n\nDetailed Attendance\n';
                csv += 'Session,Member Name,Email,Method,Marked At\n';
                reportData.forEach(record => {
                    record.attendance.forEach(attendee => {
                        csv += `"${record.session}",`;
                        csv += `"${attendee.userId?.name || 'Unknown'}",`;
                        csv += `"${attendee.userId?.email || ''}",`;
                        csv += `"${attendee.method}",`;
                        csv += `"${new Date(attendee.markedAt).toLocaleString()}"\n`;
                    });
                });
                ...
                link.href = url;
                link.download = `smartattend-report-${new Date().toLocaleDateString()}.csv`;
        ...
        return sessionMembers.filter(member => (
            ...
            { activeView === 'Sessions' && (
                <>Showing {filteredSessionsView.length} of {sessions.length} sessions</>
            )}
                                    { activeView === 'Members' && (
            <>Showing {filteredMembersView.length} of {members.length} members</>
        )}
{
    activeView === 'Reports' && (
        <>Showing {filteredSessionOptions.length} results for "{searchQuery}"</>
    )
}
        ));
